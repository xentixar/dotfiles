...
updatebars(void)
{
	...
	Atom xa = XInternAtom(dpy, "_NET_WM_WINDOW_OPACITY", False);
	unsigned long opacity = (unsigned long)(0xffffffff * baralpha);
	
	for (m = mons; m; m = m->next) {
		if (m->barwin) {
			/* Update opacity for existing bar */
			if (xa != None)
				XChangeProperty(dpy, m->barwin, xa, XA_CARDINAL, 32, PropModeReplace,
		...
		/* Set window opacity for transparency */
		if (xa != None)
			XChangeProperty(dpy, m->barwin, xa, XA_CARDINAL, 32, PropModeReplace,
					(unsigned char *)&opacity, 1);
	}
